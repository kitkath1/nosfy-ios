...
export const ETAPES_QA: EtapeQA[] = [
  ...
  {"id": "qa-14-premiere-seance", "n": 14, "titre": "Compte neuf : première séance jusqu’aux stories", "geste": "Sur un compte Apple dédié au test neuf, finir une séance avec une série réelle, suivre les annonces et la story, ouvrir le coffre ; relancer puis revenir le lendemain.", "front": "Au départ, collection et gains vides. Après la séance, ses propres pièces et son sachet, annonces cohérentes, story adaptée, widgets mis à jour ; gains conservés à la relance, accueil du lendemain cohérent. Tirage de carte qualifié avec la session Forge.", "back": "Même identité dans profil, workout, séries, coffre et faits ; aucun doublon au rejeu. Vérifier la reprise après coupure réseau. La nouvelle garde d’intégrité devra refuser une séance absente sans perdre les gains différés.", "frontVerdict": {"etat": "a_valider"}, "backVerdict": {"etat": "a_valider", "le": "18-09", "note": "Banc nominal --flow : 50 PASS, sans rendu ni feuille Apple. Nouveau banc --integrite : 32 PASS, 1 FAIL (séance absente rémunérée). Parcours iPhone et retour du lendemain non mesurés. tools/production/ETAT-PRODUCTION-2026-09-18.md"}},
  {"id": "qa-15-compte-existant", "n": 15, "titre": "Compte existant : conserver et retrouver son historique", "geste": "Après la session chauffe, comparer la session iPhone au profil serveur et inventorier les séances locales, synchronisées et en attente. Préserver l’historique avant tout test de déconnexion ; vérifier ensuite relance et reconnexion.", "front": "Les séances, pièces, sachets et cartes de ce compte restent les siennes. L’inscription ne rejoue pas ; les données d’un autre compte n’apparaissent pas.", "back": "Identité identique sur iPhone et serveur ; différences d’historique expliquées, file de synchronisation rattrapée, aucune écriture attribuée au mauvais compte. Le test n’efface pas le compte personnel.", "frontVerdict": {"etat": "a_valider"}, "backVerdict": {"etat": "a_valider", "le": "18-09", "note": "Lecture seule : le profil Apple au prénom connu porte une séance ouverte, aucune terminée ; préfixe différent du test du 14-09. Les ~38 séances du téléphone ne sont pas réconciliées. Ce constat ne prouve pas une perte. tools/production/preuves-2026-09-18/backend.json"}},
  {
    ...
    backVerdict: { etat: 'a_valider', note: '34 contrôles du calcul Swift PASS ; confirmer la correspondance compte/serveur lors du parcours iPhone.' },
  },
]
