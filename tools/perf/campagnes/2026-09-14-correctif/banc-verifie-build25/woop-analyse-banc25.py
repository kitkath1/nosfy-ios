...
for i,group in enumerate(groups):
 ...
 stable=[r for r in group if r['t']>=start+warmup and r['onglet']=='home' and not any(r[k] for k in ['seance','player','drag','chemin']) and not r.get('welcome',0) and not r.get('premiere',0)]
...
result={'vol':str(vol),'nav':str(nav),'phases_attendues':expected,'parcours_complet':[g[0]['bancHome'] for g in groups]==expected,'absence_popup_instrumentee':all('welcome' in r and 'premiere' in r for r in rows),'resultats':summary,'evenements_banc':[e for e in events if 'banc-home' in e.get('evenement','')],'limites':'CPU Mach récent, callbacks CADisplayLink, pas énergie ni images GPU. Phases chaudes avec fond protégé potentiellement immobile.'}
