    ...
    private var mainBody: some View {
        ZStack {
            ...
            .onChange(of: active != nil, initial: true) { _, enSeance in
                ...
                // le Dynamic Island »). Et LA PASTILLE VIT dans toute
                // l'app (06-09, après le malentendu du Foyer : « la
                // pastille vit ! tap sur l'île, elle sort ; je peux la
                // cacher ») — seule la home noire « session en cours »
                // (le Foyer, l'autre session) ne l'affichera pas, chez
                // ELLE.
            ...
            if filmNosfy {
                NosfyOnboarding { reponses in
                    print("[NOSFY] langue=\(reponses.langue) prenom=\(reponses.prenom ?? "—") but=\(reponses.but ?? "—") objectif=\(reponses.objectifHebdo.map(String.init) ?? "—")")
                    ecrireProfil(reponses)        // avec `-sessionBanc` : la mesure de definir_profil
