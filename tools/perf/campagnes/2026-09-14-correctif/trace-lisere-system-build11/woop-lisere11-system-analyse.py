...
P=Path('/private/tmp')
def parse(schema):
 root=E.parse(P/f'woop-lisere11-system-{schema}.xml').getroot()
...
main={k:[r for r in rs if 'Main Thread (0x158ec7)' in (r.get('thread') or '')] for k,rs in allrows.items()}
...
for h in main['potential-hangs']:
 ...
 h['narratives']=narr
 print(json.dumps(h))
(P/'woop-lisere11-system-analysis.json').write_text(json.dumps(result,indent=2))
(P/'woop-lisere11-system-main-resolved.json').write_text(json.dumps(main,indent=2))
