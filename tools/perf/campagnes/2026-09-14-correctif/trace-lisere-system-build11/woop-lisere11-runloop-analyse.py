...
import json
BASE=Path('/private/tmp');PID='17521';TID='1412807'
def load(name):
...
rl,rr=load('woop-lisere11-system-runloop-events.xml');h,hr=load('woop-lisere11-system-potential-hangs.xml')
...
(BASE/'woop-lisere11-runloop-analysis.json').write_text(json.dumps(summary,indent=2,ensure_ascii=False));(BASE/'woop-lisere11-runloop-intervals.json').write_text(json.dumps({'individual_iteration':records(iters),'waiting_for_events':records(waits),'outside_waiting_for_events':records(busy)},indent=2))
