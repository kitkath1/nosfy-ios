...
import json
p=Path('/private/tmp/woop-profil-cpu-build5-samples.xml')
...
Path('/private/tmp/woop-profil-cpu-analysis.json').write_text(json.dumps(meta,indent=2))
Path('/private/tmp/woop-profil-cpu-resolved-samples.json').write_text(json.dumps(samples))
