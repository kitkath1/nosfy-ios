...
P=Path('/private/tmp')
spec=importlib.util.spec_from_file_location('a',P/'woop-home15-full-system-analyse.py');m=importlib.util.module_from_spec(spec)
...
(P/'woop-home15-full-system-sync-groups.json').write_text(json.dumps(groups,indent=2))
(P/'woop-home15-full-system-sync-summary.json').write_text(json.dumps(summary,indent=2))
(P/'woop-home15-full-system-workers.json').write_text(json.dumps(workers,indent=2))
v=[json.loads(s) for s in (P/'woop-build15-home-froid-vol.jsonl').read_text().splitlines() if s.strip()]
for name,rs in [('cold20plus',[r for r in v if r['therm']==0 and r['t']>=20 and not r.get('protection')]),('protected49plus',[r for r in v if r['t']>=49 and r.get('protection')])]:
 print(name,len(rs),'t',rs[0]['t'],rs[-1]['t'],'cpu',statistics.median(r['cpu'] for r in rs),'img',statistics.median(r['img'] for r in rs),'worstmax',max(r['pire'] for r in rs),'maxgapmedian',statistics.median(r['pire'] for r in rs))
