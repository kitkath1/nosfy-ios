...
P=Path('/private/tmp')
def parse(schema):
 root=E.parse(P/f'woop-home15-full-system-{schema}.xml').getroot()
...
main={k:[r for r in rs if 'Main Thread (' in (r.get('thread') or '') and '(Woop, pid: 17908)' in (r.get('thread') or '')] for k,rs in allrows.items()}
...
for h in main['potential-hangs']:
 ...
 h['narratives']=narr
 print(json.dumps(h))
(P/'woop-home15-full-system-analysis.json').write_text(json.dumps(result,indent=2))
(P/'woop-home15-full-system-main-resolved.json').write_text(json.dumps(main,indent=2))
