...
out=Path('/private/tmp/woop-validation33'); out.mkdir(exist_ok=True)
...
info=plistlib.loads((app/'Info.plist').read_bytes()); assert info['CFBundleVersion']=='33', info['CFBundleVersion']
events=[]
...
try:
 for nom,extra in [('normale',['-sondeAnimations']),('sans-perle',['-sansSoufflePerle'])]:
  run(nom+'-lancement',['xcrun','devicectl','device','process','launch','--terminate-existing','--device',core,'--timeout','20',bundle,'--','-sondeVol','-ecranEveille','-navProbe','-openTab','home']+extra,35)
  print(nom,'observation35s, protection normale',flush=True);time.sleep(35)
  run(nom+'-collecte',['python3','/private/tmp/woop-collect-id22.py','woop-home33-'+nom,'--nav'],35)
  print((out/(nom+'-collecte.log')).read_text()[:1300],flush=True)
finally:
 launch('home-finale')
