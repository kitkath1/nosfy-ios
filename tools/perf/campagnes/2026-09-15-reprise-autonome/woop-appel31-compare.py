...
out=Path('/private/tmp/woop-appel31-compare'); out.mkdir(exist_ok=True)
...
info=plistlib.loads((app/'Info.plist').read_bytes()); assert info['CFBundleVersion'] in ('31','32'), info['CFBundleVersion']
events=[]
...
try:
 for nom,extra in [('avec',[]),('sans',['-sansAppelChapitre'])]:
  ...
  run(nom+'-collecte',['python3','/private/tmp/woop-collect-id22.py','woop-appel31-'+nom,'--nav'],35)
finally:
 launch('home-finale')
