...
out=Path('/private/tmp/woop-validation34'); out.mkdir(exist_ok=True)
...
try:
 ...
 for i in range(6):
  time.sleep(30)
  if i in (0,2,5):
   run('collecte-'+str(i),['python3','/private/tmp/woop-collect-id22.py','woop-home34-'+str(i),'--nav'],35)
   print((out/('collecte-'+str(i)+'.log')).read_text()[:1100],flush=True)
finally:
 launch('home-sans-sonde')
ui('pull-chapitre','testRetourPullEtChapitre')
run('collecte-pull',['python3','/private/tmp/woop-collect-id22.py','woop-pull34','--nav-only'],35)
