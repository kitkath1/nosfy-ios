...
out=Path('/private/tmp/woop-balade34'); out.mkdir(exist_ok=True)
...
info=plistlib.loads((app/'Info.plist').read_bytes()); assert info['CFBundleVersion']=='34', info['CFBundleVersion']
events=[]
...
try:
 run('lancement-sonde',['xcrun','devicectl','device','process','launch','--terminate-existing','--device',core,'--timeout','20',bundle,'--','-sondeVol','-ecranEveille','-navProbe','-openTab','home'],30)
 time.sleep(16)
 run('navigation-repetee',['xcodebuild','test-without-building','-xctestrun','/private/tmp/woop-nav-runtime-qa/dd/Build/Products/NavRuntimeUITests_iphoneos26.5-arm64.xctestrun','-only-testing:NavRuntimeUITests/NavRuntimeUITests/testAllersRetoursMesures','-destination','platform=iOS,id='+udid,'-destination-timeout','15','-parallel-testing-enabled','NO','-maximum-concurrent-test-device-destinations','1','-resultBundlePath',str(out/'navigation-repetee.xcresult')],130)
 print('Home après balade : observation90s',flush=True)
 for i in range(3): time.sleep(30)
 run('collecte-mesure',['python3','/private/tmp/woop-collect-id22.py','woop-balade34','--nav'],35)
finally:
 launch('retour-normal-sans-sonde')
run('collecte-restauration',['python3','/private/tmp/woop-collect-id22.py','woop-balade34-restauration','--nav-only'],35)
