...
final class NavRuntimeUITests: XCTestCase {
    ...
    func testGestesDuComposantSurIPhone() throws {
        ...
        if ProcessInfo.processInfo.environment["WOOP_HALO_BANC"] == "swiftui" {
            app.launchArguments.append("-ileHaloSwiftUI")
        }
        if ProcessInfo.processInfo.environment["WOOP_HALO_BANC"] == "discret" {
            app.launchArguments.append("-ileHaloDiscret")
        }
        app.terminate()
        app.launch()
        func element(_ id: String) -> XCUIElement {
    ...
    @MainActor
    func testPriseLateraleSurIPhone() throws {
        continueAfterFailure = false
        let app = XCUIApplication(bundleIdentifier: "fr.kathryn.woop")
        app.launchArguments = ["-piluleLab", "-sansSondeVol", "-ecranEveille"]
        app.terminate()
        app.launch()
        func element(_ id: String) -> XCUIElement {
            app.descendants(matching: .any).matching(identifier: id).firstMatch
        ...
        let pilule = element("seance-pastille")
        let ile = element("seance-ile")
        XCTAssertTrue(pilule.waitForExistence(timeout: 15))
        point(pilule.frame.midX, pilule.frame.midY).press(forDuration: 0.02,
            thenDragTo: point(app.frame.midX, 5), withVelocity: 2400, thenHoldForDuration: 0)
        XCTAssertTrue(ile.waitForExistence(timeout: 7))
        Thread.sleep(forTimeInterval: 1)
        let avant = XCTAttachment(screenshot: app.screenshot())
        avant.name = "prise-laterale-avant"
        avant.lifetime = .keepAlways
        add(avant)
        // Exactement sur le chrono, à hauteur du capteur, sans viser y57.
        point(app.frame.midX - 96, 33).press(forDuration: 0.08,
            thenDragTo: point(app.frame.midX - 96, 250),
            withVelocity: .slow, thenHoldForDuration: 0.3)
        XCTAssertTrue(pilule.waitForExistence(timeout: 5), "La sortie depuis le chrono ne répond pas")
        let apres = XCTAttachment(screenshot: app.screenshot())
        apres.name = "prise-laterale-sortie"
