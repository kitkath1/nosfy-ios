...
final class NavRuntimeUITests: XCTestCase {
    @MainActor
    func testPalierParoleVisible() throws {
        ...
        func phrase() -> String {
            app.otherElements.matching(NSPredicate(format: "label CONTAINS %@", "Kathryn")).allElementsBoundByIndex.first {
                $0.frame.minY < 300 && $0.frame.height > 90 && $0.frame.height < 210 && $0.label.split(separator: " ").count > 5
            }?.label ?? ""
        }
        let limite = Date().addingTimeInterval(310)
        var avant = phrase()
        while !avant.contains("4 minutes") && Date() < limite {
            Thread.sleep(forTimeInterval: 10)
            avant = phrase()
        }
        XCTAssertTrue(avant.contains("4 minutes"), avant)
        try app.screenshot().pngRepresentation.write(to: dossier.appendingPathComponent("palier-4min.png"))
        Thread.sleep(forTimeInterval: 65)
        let apres = phrase()
        XCTAssertTrue(apres.contains("5 minutes"), apres)
        XCTAssertNotEqual(avant.replacingOccurrences(of: "4 minutes", with: "minutes"),
                          apres.replacingOccurrences(of: "5 minutes", with: "minutes"))
        try app.screenshot().pngRepresentation.write(to: dossier.appendingPathComponent("palier-5min.png"))
        print("WOOP_PALIER_VISIBLE_OK | \(avant) | \(apres)")
    }

    @MainActor
    func testPanneauProfilPret() throws {
        continueAfterFailure = false
        let app = XCUIApplication(bundleIdentifier: "fr.kathryn.woop")
        app.activate()
        if app.buttons["Later"].waitForExistence(timeout: 6) { app.buttons["Later"].tap() }
        XCTAssertTrue(app.staticTexts["Locked"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.buttons["Réglages"].exists)
        let dossier = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        for i in 0..<3 {
            try app.screenshot().pngRepresentation.write(to: dossier.appendingPathComponent("panneau-profil-\(i).png"))
            Thread.sleep(forTimeInterval: 0.7)
        }
        print("WOOP_PANNEAU_PROFIL_PRET")
    }

    @MainActor
    func testParolesAuxRetours() throws {
        continueAfterFailure = false
        let app = XCUIApplication(bundleIdentifier: "fr.kathryn.woop")
        app.activate()
        if app.buttons["Later"].waitForExistence(timeout: 6) { app.buttons["Later"].tap() }
        let dossier = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        func phrase() -> String {
            let candidates = app.otherElements.matching(NSPredicate(format: "label CONTAINS %@", "Kathryn")).allElementsBoundByIndex
            return candidates.first { $0.frame.minY < 300 && $0.frame.height > 90 && $0.frame.height < 210 && $0.label.split(separator: " ").count > 5 }?.label ?? ""
        }
        Thread.sleep(forTimeInterval: 4)
        let avant = phrase()
        XCTAssertFalse(avant.isEmpty, app.debugDescription)
        ...
        for i in 0..<4 {
            try app.screenshot().pngRepresentation.write(to: dossier.appendingPathComponent("parole-retour-\(i).png"))
            Thread.sleep(forTimeInterval: 0.35)
        }
        Thread.sleep(forTimeInterval: 3)
        let retourExos = phrase()
        XCTAssertFalse(retourExos.isEmpty)
        XCTAssertNotEqual(avant, retourExos, "Le retour doit choisir une autre variante")
        app.buttons["person"].tap()
        XCTAssertTrue(app.buttons["Réglages"].waitForExistence(timeout: 5))
        app.buttons["Retour"].tap()
        Thread.sleep(forTimeInterval: 4)
        let retourProfil = phrase()
        XCTAssertFalse(retourProfil.isEmpty)
        XCTAssertNotEqual(retourExos, retourProfil)
        print("WOOP_PAROLES_RETOURS_OK | \(avant) | \(retourExos) | \(retourProfil)")
        try app.screenshot().pngRepresentation.write(to: dossier.appendingPathComponent("parole-retour-net.png"))
    }
    ...
    func testEcranNuPretPourMesure() throws {
        ...
        XCTAssertNotEqual(app.state, .notRunning)
        app.activate()
        if app.buttons["Later"].waitForExistence(timeout: 6) {
