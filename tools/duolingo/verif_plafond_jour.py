#!/usr/bin/env python3
"""Le plafond du jour (2 séances comptées par jour local) — joué ENTIER dans BEGIN / ROLLBACK
sur la base vivante, par l'API de gestion : rien ne reste. `--avec-migration` rejoue d'abord
la migration 20260920160000 (la répétition d'avant la pose) ; sans, elle doit être posée."""
import json, sys, urllib.request, urllib.error
from pathlib import Path
R = Path(__file__).resolve().parents[2]
REF = 'ytnnyjkramgiqyxdrkcu'
MIGRATION = R / 'supabase/migrations/20260920160000_chemin_plafond_deux_seances_par_jour.sql'
BANC = R / 'tools/duolingo/qa-plafond-jour.sql'
...
print(f"Plafond du jour : {len(lignes) - ko} PASS, {ko} FAIL" + (' (avec la migration rejouée, transaction annulée)' if avec else ' (migration posée, transaction annulée)'))
