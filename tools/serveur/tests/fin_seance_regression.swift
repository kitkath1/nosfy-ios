...
actor SupabaseSync {
 ...
 var suspendre=false;var pause:CheckedContinuation<Void,Never>?;var lots:[[UUID]]=[]
 func bloquer(){suspendre=true}
 func liberer(){suspendre=false;pause?.resume();pause=nil}
 func push(_ s:[Snapshot],proprietaire:String?=nil)async {
  if !s.isEmpty {
   envois+=1;lots.append(s.map(\.id));fileAvantReseau=await !OutboxGains.shared.seancesARejouer.isEmpty
   if suspendre { await withCheckedContinuation { pause=$0 } }
  }
 }
}
...
@main struct Tests {
 ...
 @MainActor static func main()async throws {
  ...
  check(w.totalVolume==50,"volume de la Home : séries prévues exclues")
  check(w.orderedExercises[0].maxWeight==5,"charge maximale : aucune série prévue transformée en record")
  ...
  w.recompenseARegler=true;try ctx.save()
  await SupabaseSync.shared.bloquer()
  let avant=await SupabaseSync.shared.envois
  let premiere=Task { @MainActor in await ReglementSeance.shared.reprendre() }
  for _ in 0..<100 {
   if await SupabaseSync.shared.envois > avant { break }
   try await Task.sleep(for:.milliseconds(10))
  }
  empty.recompenseARegler=true;try ctx.save()
  let seconde=Task { @MainActor in await ReglementSeance.shared.reprendre() }
  try await Task.sleep(for:.milliseconds(30))
  check(await SupabaseSync.shared.envois==avant+1,"deux reprises ne poussent pas en concurrence")
  await SupabaseSync.shared.liberer();await premiere.value;await seconde.value
  check(await SupabaseSync.shared.lots.last?.contains(empty.remoteID)==true,"séance terminée pendant une reprise traitée ensuite")
  ReglementSeance.shared.recevoir(c)
