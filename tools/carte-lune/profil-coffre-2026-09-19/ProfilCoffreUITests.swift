        ...
        for langue in ["fr", "en"] {
            ...
            for (id, fr, en) in pages {
                ...
                if id == "argent" {
                    ...
                    let candidats = page.buttons.matching(NSPredicate(format: "label CONTAINS %@ AND NOT label CONTAINS %@", langue == "fr" ? "pièces —" : "coins —", "silver"))
                    XCTAssertEqual(candidats.count, 1)
                    pill = candidats.firstMatch
