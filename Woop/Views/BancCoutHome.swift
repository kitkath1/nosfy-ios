...
final class BancCoutHome {
    ...
    func executer() async {
        ...
        do {
            ...
            if CommandLine.arguments.contains("-bancFroidHome") {
                guard try await attendreFroid() else { return }
            }
    ...
    @MainActor
    private func attendreFroid() async throws -> Bool {
        // Le téléphone se repose sur le châssis sans décors. La réduction
        // expire sur l'appareil : une coupure du câble ne laisse pas un écran nu.
        poser(.nu)
        var secondesNominales = 0
        for seconde in 0..<300 {
            guard verifierContexte() else { return false }
            let nominal = ProcessInfo.processInfo.thermalState == .nominal
            secondesNominales = nominal ? secondesNominales + 1 : 0
            if seconde.isMultiple(of: 10) {
                NavDiagnostic.noter("banc-home-refroidissement")
            }
            if secondesNominales >= 6 {
                NavDiagnostic.noter("banc-home-depart-froid")
                return true
            }
            try await Task.sleep(for: .seconds(1))
        }
        NavDiagnostic.noter("banc-home-annule", destination: "froid-non-obtenu-en-300s")
        return false
    }
