-- LE PRIX DE LA PISCINE DANS etat_coffre — 15-09-2026 (soir)
--
-- Verdict (Kathryn, relayé par la session cardio) : la fiche piscine affiche « +20 par longueur »
-- et un plafond ; elle doit LIRE ce prix au serveur, pas le connaître en dur (la loi des prix :
-- ils vivent en base, l'app les lit — jamais une constante Swift qui double une règle serveur).
--
-- Les deux clés existent DÉJÀ dans reward_rules depuis 20260915160000 (pieces_par_longueur = 20,
-- cardio_piscine_max = 300) et tout le barème de pieces_cardio_seance les lit via regle_num. Il ne
-- manquait que de les RENDRE au client, comme pieces_par_serie. L'app les décode en OPTIONNELS
-- (clé absente = secours 20 / 300) — donc « écrire avant lire » est respecté : cette pose ne casse
-- rien, elle remplit un secours.
--
-- create-or-replace de la fonction VIVANTE (pg_get_functiondef, 15-09 soir) : on n'ajoute que les
-- deux lignes, tout le reste est recopié à l'identique. Vérif : POST /rpc/etat_coffre (jeton du
-- compte de test) → pieces_par_longueur 20, cardio_piscine_max 300.
